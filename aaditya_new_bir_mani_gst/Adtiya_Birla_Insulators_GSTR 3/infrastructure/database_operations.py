...
class database_operations:
        ...
        try:
            ...
            query = """
                    ...
                    FROM
                         gstr1_data_1
                    WHERE
                        STATUS IS NULL OR status = '' OR status = 'FAILED' OR STATUS = 0;
                    """
    ...
    def fetch_data_from_database_GSTR2(conn):
        try:
            ...
            query = ''' SELECT
                    ...
                    STATUS
                FROM gstr4_data_1
                WHERE STATUS IS NULL OR STATUS = '' OR STATUS = 'FAILED' OR STATUS = 0;
                '''
    ...
    #new funtion to update the column status based on error that we got from the ingestion API response.
    def GSTR_1_update_status_for_document_numbers(conn, document_numbers):
        try:
            cursor = conn.cursor()

            if not document_numbers:
                print("📌 No invalid document numbers provided. Updating all rows to '1'.")
                update_query = "UPDATE gstr1_data_1 SET Status = %s"
                cursor.execute(update_query, ('1',))
                conn.commit()
                print(f"✅ Updated all rows to status '1'. Total rows affected: {cursor.rowcount}")
            else:
                # Step 1: Update provided (invalid) document numbers to '0'
                placeholders_invalid = ','.join(['%s'] * len(document_numbers))
                update_invalid_query = f"""
                UPDATE gstr1_data_1
                SET Status = '0'
                WHERE DOCUMENTNUMBER IN ({placeholders_invalid})
                """
                cursor.execute(update_invalid_query, document_numbers)
                print(f"⚠️ Updated {cursor.rowcount} rows to status '0' (invalid).")

                # Step 2: Update the rest to '1'
                placeholders_valid = ','.join(['%s'] * len(document_numbers))
                update_valid_query = f"""
                UPDATE gstr1_data_1
                SET Status = '1'
                WHERE DOCUMENTNUMBER NOT IN ({placeholders_valid})
                """
                cursor.execute(update_valid_query, document_numbers)
                print(f"✅ Updated {cursor.rowcount} rows to status '1' (valid).")

                conn.commit()

            cursor.close()
        except Exception as e:
            logging.error("Error updating document status: %s", e)
            if conn:
                try: conn.rollback()
                except: pass
    ...
    def GSTR_2_update_status_for_document_numbers(conn, document_numbers):
        try:
            cursor = conn.cursor()

            if not document_numbers:
                print("📌 No invalid document numbers provided. Updating all rows to '1'.")
                update_query = "UPDATE gstr4_data_1 SET Status = %s"
                cursor.execute(update_query, ('1',))
                conn.commit()
                print(f"✅ Updated all rows to status '1'. Total rows affected: {cursor.rowcount}")
            else:
